...
* **Decision**: Implement a .NET build-time aggregation step triggered during `dotnet publish` (and hooked into local Aspire watch workflows). This step compiles all folder-based metadata and Markdown content into a single `resume.json` artifact served from `wwwroot/data/`.
