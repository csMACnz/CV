...
public class DebugTracerTests
{
    ...
    [Fact]
    public void Debug_AppHost_Build_SucceedsWithoutError()
    {
        using var result = BuildHelpers.RunDotnetBuildAppHost("Debug");

        Assert.True(result.Succeeded,
            $"AppHost Debug build failed (exit code {result.ExitCode}).\nOutput:\n{result.Output}\nError:\n{result.Error}");
    }
}
